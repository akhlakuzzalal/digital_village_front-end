      ...
      {news.map((data) => (
        ...
        <div className="flex flex-col flex-grow">
          ...
          <div className="flex flex-col space-y-1 my-4">
            <h2 className="text-gray-700 font-primary text-lg">{data?.title.slice(0,30)}...</h2>
            <p className="text-gray-500 text-sm">{data?.description.slice(0,30)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-3">
          ...
          <button onClick={()=>handleDelete(data._id)} className="w-100 flex items-center justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            <FaTrashAlt />
          </button>
        </div>
      </div>
